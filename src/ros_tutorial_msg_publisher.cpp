...
#include "irvs_ros_tutorials/msgTutorial.h"          // msgTutorialメッセージファイルのヘッダー（ビルド後に自動的に生成される）
...
int main(int argc, char **argv)                         // ノードのメイン関数
{
  ...
  // 発行者の宣言、 irvs_ros_tutorialsパッケージのmsgTutorialメッセージファイルを利用した
  ...
  while (ros::ok())
  {
    irvs_ros_tutorials::msgTutorial msg;      // msgTutorialメッセージファイル形式でmsgというメッセージを宣言
